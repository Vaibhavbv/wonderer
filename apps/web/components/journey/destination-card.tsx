  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-y-3 text-white/75 text-[13px]">
        <Row icon={<Calendar className="h-4 w-4" />} label={dest.date} />
        <Row icon={<Clock className="h-4 w-4" />} label={`${dest.days} days`} />
        <Row icon={<Heart className="h-4 w-4" />} label={`${dest.memories} memories`} />
        <Row icon={<CloudSun className="h-4 w-4" />} label={dest.weather} />
      </div>
