...
- **Personal homepage** — `app/page.tsx` branches on auth (env-guarded so the site boots without Clerk): signed-out gets the demo journey + the formerly orphaned marketing sections + CTA; signed-in gets greeting, own stats, continue-draft card, and a feed from followed travelers (`GET /v1/feed`, first consumer) with load-more and a personality empty state.
