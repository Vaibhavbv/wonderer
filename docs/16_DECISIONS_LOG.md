...
### ADR-018 — Single dark theme + Radix/shadcn-style primitives (user-mandated brand change)
- **Date:** 2026-07-12 · **Status:** Accepted
- **Context:** The UI was visually inconsistent — the trip view read premium while other pages were flat coral-on-white with raw `bg-white`/`neutral`/green/yellow utilities, three duplicated input styles, mixed radii, and two dark-band recipes. The user explicitly chose a modern dark theme app-wide plus shadcn/ui.
- **Decision:** (a) Flip the semantic token VALUES in `globals.css` to a warm-charcoal dark palette (names unchanged; brand 50–900 ramps untouched) so token-clean components convert automatically; `color-scheme: dark` set statically — single theme, no toggle. (b) Pale ramp fills become alpha tints (`bg-primary-500/15 text-primary-400` idiom). (c) shadcn-style primitives (Dialog/Input/Textarea/Label/Badge/Separator) hand-written against OUR semantic tokens with Radix deps from npm (registry fetch proxy-blocked); kept the existing cva Button/Card APIs and re-skinned them. (d) Radius rule: buttons/inputs `rounded-lg`, cards `rounded-xl`, modals/banners `rounded-2xl`, pills/avatars `rounded-full`. (e) Dark bands (footer/CTA/pricing highlight) became elevated/hairline sections; Clerk renders dark via `@clerk/themes`. The wander view and photo-overlay glass classes are deliberate exceptions.
- **Tradeoffs:** ➖ touches ~40 files; ➕ one source of surface/text color, primitives kill the duplicated input/modal/badge styles, and future components inherit the theme for free.

*(Add ADR-019+ below as decisions are made.)*
